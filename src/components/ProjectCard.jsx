    ...
    >
      ...
      <div className="relative h-48 w-full overflow-hidden rounded-xl">
        ...
        <div className="absolute inset-0 flex items-center justify-center gap-4 bg-black/40 opacity-0 backdrop-blur-sm transition-opacity duration-300 group-hover:opacity-100">
          <a href={project.github} className="rounded-full bg-white/20 p-2 text-white hover:bg-white/40">
            <FaGithub size={20} />
          </a>
          <a href={project.demo} className="rounded-full bg-white/20 p-2 text-white hover:bg-white/40">
      ...
      <div className="mt-4">
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          {project.tags.map((tag) => (
            <span
              key={tag}
              className="rounded-md bg-white/5 px-2 py-1 text-[10px] text-gray-300 border border-white/10"
            >
              {tag}
            </span>
          ))}
